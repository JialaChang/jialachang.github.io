...
function decideProvinceOwner(id) {
    for (const nationId of Object.keys(NATION_START)) {
        if (NATION_START[nationId].has(id))
            return nationId;
    }
...
class Nation {
    ...
    overseaCapital;
    captialIndex = 0;
    constructor(id, capitalChain, overseaCapital // 圖外首都，不可互動
    ) {
        this.id = id;
    ...
    constructor(nations) {
        ...
        for (const s of MAP_DATA.shapes) {
            if (s.id in DORMANT_MUNICIPALITY)
                continue;
            this.provinces.set(s.id, new Province(s.id, s.cx, s.cy, JAPAN_START.has(s.id) ? "jap" : "roc", INITIAL_ARMIES[s.id] ?? 0));
        }
    }
}
// SVG 地圖
